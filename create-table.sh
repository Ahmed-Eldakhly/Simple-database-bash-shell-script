...
columnDatatype=""
#------------------------------------------------------------------------------------
#function to check if the table is exist and not contain spaces 
function thisCheckOnTableCreation(){
	read -p "please insert table name to be created: " tableName
	while [[ -f $tableName || `echo $tableName | wc -w` > 1 ]] 
	do
		echo -e "\e[31mThe table name already exist or contain spaces, please insert another name.\e[0m"
...
#------------------------------------------------------------------------------------
#show the perivious columns with datatype to the user while adding new column.
...
function createTable() {
	...
	endCreationFlag=1;
	#loop to keep adding columns till the user finish.
	...
	do
		...
		then
			...
			while [[ `echo $newColumnName | wc -w` > 1 ]]
			do
